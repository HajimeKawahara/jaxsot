...
pg = jaxopt.ProximalGradient(fun=objective,
                             prox=jaxopt.prox.prox_non_negative_lasso,
                             maxiter=5000)
res = pg.run(init_params=np.random.normal(0.0, 1.0, npix),
             hyperparams_prox=0.01,
             lamtsv=0.01)
params, state = res
